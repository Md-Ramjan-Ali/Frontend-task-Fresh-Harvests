...
export default function Navbar({ openNav, onProfileClick }: props) {
  ...
  return (
    <div
      className={` fixed w-full transition-all duration-300 h-[12vh] z-[1000] ${bgClass}`}
    >
      <div className="flex justify-between items-center mx-auto h-full w-[95%] lg:w-[90%]">
        ...
        <div className="flex items-center space-x-1">
          <button
            className={`px-4 py-2 hidden lg:flex rounded-lg font-medium  lg:items-center gap-1 cursor-pointer hover:text-[#FF6A1A] ${pathname === "/" && !navBg
          ...
          >
            <FaHeart
              size={18}
              className={`  ${pathname === "/" && !navBg
                ? "text-white hover:text-[#FF6A1A] bg-transparent"
                : "text-[#749b3f] hover:text-[#FF6A1A] bg-white"
                } `}
            />
            Favorite
          </button>
          ...
          <button
            className={`relative px-4 py-2 rounded-lg font-medium flex items-center gap-2 cursor-pointer hover:text-[#FF6A1A]  ${pathname === "/" && !navBg
              ? "text-white bg-transparent"
              : "text-black bg-white"
              }`}
          >
            ...
            <div className="relative">
              <FaCartShopping
                size={22}
                className={`  ${pathname === "/" && !navBg
                  ? "text-white bg-transparent hover:text-[#FF6A1A]"
                  : "text-[#749b3f]  hover:text-[#FF6A1A] bg-white"
                  } `}
              />
          ...
          {/* Show Avatar if logged in, otherwise show Sign In button */}
          {session ? (
            <div className="hidden lg:block">
              <Avatar onProfileClick={handleProfileClick} />
            </div>
          ) : (
            <button
